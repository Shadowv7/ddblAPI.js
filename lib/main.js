...
class ddblAPI {
  ...
  postStats(id, serverCount) {
    ...
    if(!Number(serverCount)) throw new Error("[DDBL-MODULE][POST-STATS] serverCount is not a number.");
      return new Promise((resolve, reject) => {
        axios({
          ...
          .then(() => {
            ...
            .catch((err) => {
              if(err) reject(new Error(`[DDBL-MODULE][POST-STATS] Error:\n${err}`));
            });
      });
  }
      ...
      return new Promise((resolve, reject) => {
        axios({
          ...
          .then(async(res) => {
            ...
            .catch((err) => {
              if(err) reject(new Error(`[DDBL-MODULE][GET-STATS] Error:\n${err}`));
            });
      });
  }
      ...
      return new Promise((resolve, reject) => {
        axios({
          ...
          .then(async(res) => {
            ...
            await resolve(res.data.votes);
          })
            .catch((err) => {
              if(err) reject(new Error(`[DDBL-MODULE][GET-VOTES] Error:\n${err.message}`));
            });
      });
  }
